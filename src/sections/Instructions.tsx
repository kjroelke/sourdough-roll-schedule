...
export default function Instructions({ endTime, readyTime, servings }) {
		...
		setInstructions((prev) => [
			{
				time: endTime
					...
					.subtract(1, 'hour')
					.subtract(30, 'minutes'),
				instruction: prev[0].instruction,
				description: `${readyTime} hours before you plan to mix the dough, add the ingredients to make ${Math.round(
					(100 / 12) * servings,
				)}g of active sourdough starter to a clean jar. Stir until combined, loosely cover the jar and let the starter rise at room temperature. (The ingredients will make a little more than needed.) The sourdough starter is ready to use when it has doubled in size and there are plenty of bubbles on the surface and sides of the jar.`,
			},
			{
				time: endTime
					.subtract(30, 'minutes')
					...
					.subtract(90, 'minutes')
					.subtract(1, 'hour')
					.subtract(30, 'minutes'), // - 30 minutes
				instruction: prev[1].instruction,
			...
			},
			{
				time: endTime
					...
					.subtract(2, 'hours')
					.subtract(90, 'minutes')
					.subtract(1, 'hour'), // - 1 hour
				instruction: prev[2].instruction,
			...
			},
			{
				time: endTime
					...
					.subtract(4, 'hours')
					.subtract(2, 'hours')
					.subtract(90, 'minutes'), // -90min + 2 hours
				instruction: prev[3].instruction,
				description: prev[3].description,
			},
			{
				time: endTime.subtract(30, 'minutes').subtract(4, 'hours'), // -3 hours
				instruction: prev[4].instruction,
				description: prev[4].description,
			},
			{
				time: endTime.subtract(30, 'minutes'),
				instruction: prev[5].instruction,
				description: prev[5].description,
			},
		]);
	}, [endTime, readyTime, servings]);
